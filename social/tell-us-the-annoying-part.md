# Tell Us the Annoying Part

Campaign draft created July 23, 2026.

## Personal LinkedIn

Graphic: `images/social/tell-us-the-annoying-part/linkedin-personal-v2.png`

LinkedIn direction: business-first, no standalone corgi imagery. The official CTF
logo appears only as a small sponsor mark.

What is the most annoying piece of software you use at work every day?

Not the dramatic outage. I mean the small thing everyone has quietly accepted.

The information you enter twice.

The screen that takes six clicks.

The spreadsheet only one person understands.

The customer question your system should answer automatically.

Tell me the annoying part.

I want to see how many of these are actually small custom app problems pretending to be permanent facts of life.

I will take some of the best examples and break down what I would build instead.

## CTF Designs LinkedIn

Graphic: `images/social/tell-us-the-annoying-part/linkedin-ctf-v3.png`

LinkedIn direction: business-first, no standalone corgi imagery. Use the official
CTF logo rather than an invented campaign mark.

The most expensive software problems are often too ordinary to get called problems.

They look like copying one lead into three systems. Chasing an approval through email. Rebuilding the same report every Friday. Explaining a confusing portal to every new customer.

Owners see the cost. Employees feel the friction. Customers experience the bad UX.

What is the tiny, annoying part of your workday that everyone has simply learned to tolerate?

Tell us the annoying part.

We will choose a few real examples and show what a cleaner workflow, interface, or small custom app could look like.

## Instagram

Graphic: `images/social/tell-us-the-annoying-part/portrait-v2.png`

The corgi-led creative is intentional here because the lighter mascot treatment
fits the platform and remains part of CTF's brand.

What tiny piece of software friction wastes your time every day?

The form that resets.
The information you enter twice.
The screen that takes six clicks.
The spreadsheet nobody trusts.
The website that makes you hunt for one simple answer.

Tell us the annoying part in the comments.

We are collecting real workplace and website frustrations, then turning some of them into practical redesign and custom app ideas.

#SmallBusiness #WebDesign #UXDesign #CustomSoftware #CTFDesigns

## Facebook

Business owners, employees, and customers all notice different problems.

An owner sees wasted labor.
An employee sees the six click workaround.
A customer sees the confusing form or booking page.

What is one small, annoying piece of software or website friction you deal with all the time?

Not a giant outage. The ordinary thing everyone has accepted even though it wastes time every day.

Tell us the annoying part. We will pick a few examples and explain what we would change or build to fix them.
